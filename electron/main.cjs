...
function registerIpc() {
  ...
  ipcMain.handle('plumbago:taxonomy-index', () => service.taxonomyIndex(requireBlog()))
  ipcMain.handle('plumbago:taxonomy-preview-change', (_event, input) => service.previewTaxonomyChange(requireBlog(), input))
  ipcMain.handle('plumbago:taxonomy-apply-change', (_event, input) => service.applyTaxonomyChange(requireBlog(), input))
  ipcMain.handle('plumbago:page-inventory', () => service.pageInventory(requireBlog()))
  ipcMain.handle('plumbago:page-preview-change', (_event, input) => service.previewPageChange(requireBlog(), input))
  ipcMain.handle('plumbago:page-apply-change', (_event, input) => service.applyPageChange(requireBlog(), input))
  ...
  ipcMain.handle('plumbago:calendar-apply-change', async (_event, input) => service.applyEditorialCalendarChange(requireBlog(), input, { githubToken: await optionalGitHubToken() }))
  ipcMain.handle('plumbago:calendar-sync-changes', async () => service.syncCalendarChanges(requireBlog(), { githubToken: await optionalGitHubToken() }))
